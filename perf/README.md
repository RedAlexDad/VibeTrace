...
| 2026-08-20 | виртуализация списка сообщений      |    4+ |  702 |   ~16633 |        394 |         2915 |
| 2026-08-20 | виртуализация + scrollToIndex       |     5 |  265 |     5245 |       1946 |         8144 |
...
### Виртуализация списка сообщений (2026-08-20)

Чат переведён на `@tanstack/react-virtual`: вместо всех ~1200 сообщений в DOM
находится только видимое окно + буфер (overscan). Прямое измерение на сессии
`ses_fe19ef…` (1196 сообщений):

| Метрика            |          До |    После |
| ------------------ | ----------: | -------: |
| Сообщений в DOM    |        1196 |    16–29 |
| DOM-узлов в чате   |      ~42300 |    ~2550 |
| Тикающие ререндеры | десятки/сек | 0 (memo) |

Начальный рендер перестал зависеть от общего числа сообщений: скорость
прокрутки и добавления новых сообщений почти постоянна на любой длине
переписки. Цена: линии-коннекторы между чатом и панелью субтасков рисуются
только для сообщений, находящихся в окне видимости (вне окна их элементов
нет в DOM).
